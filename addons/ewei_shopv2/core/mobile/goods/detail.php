...
class Detail_EweiShopV2Page extends MobilePage 
{
	public function main() 
	{
		...
		$isagent = pdo_fetchcolumn("SELECT `isagent` FROM " . tablename("ewei_shop_member") . " WHERE openid = :openid AND uniacid = :uniacid", array( ":openid" => $_W["openid"], ":uniacid" => $_W["uniacid"] ));
		...
		$isagents = m("member")->getMember($_W['openid']);
		$commission_data['isagent'] = $isagents['isagent'];
		...
		$goods["timebuy"] = "0";
		if( empty($seckillinfo) && $goods["istime"] == 1 ) 
		{
	...
	public function get_code() 
	{
		...
		$member = m("member")->getMember($openid);
		$commission_data = m("common")->getPluginset("commission");
		$img = pdo_get('ewei_shop_goods',array( "id" => $id, "uniacid" => $_W["uniacid"] ));
		$goodscode = "";
		$parameter = array( );
		if( com("goodscode") ) 
		{
			...
			if( $commission_data["codeShare"] == 1 ) 
			{
                ...
                // "portrait" => array( "thumb" => (tomedia($_W["shopset"]["shop"]["logo"]) ? tomedia($_W["shopset"]["shop"]["logo"]) : tomedia($member["avatar"])), "left" => 40, "top" => 40, "width" => 100, "height" => 100 ),
                 ...
                 "thumb" => array( "thumb" => tomedia($img["thumbs"]), "left" => 20, "top" => 20, "width" => 600, "height" => 1020 ),
                 "qrcode" => array( "thumb" => tomedia($qrcode), "left" => 457, "top" => 870, "width" => 140, "height" => 140 ),
...
//					'portrait' => array('thumb' => (tomedia($_W['shopset']['shop']['logo']) ? tomedia($_W['shopset']['shop']['logo']) : tomedia($member['avatar'])), 'left' => 40, 'top' => 40, 'width' => 100, 'height' => 100),
					//'shopname' => array('text' => $_W['shopset']['shop']['name'], 'left' => 160, 'top' => 80, 'size' => 28, 'width' => 360, 'height' => 50, 'color' => '#333'),
					//'thumb'    => array('thumb' => tomedia($goods['thumbs']), 'left' => 20, 'top' => 20, 'width' => 600, 'height' => 1020),
					//'qrcode'   => array('thumb' => tomedia($qrcode), 'left' => 457  , 'top' => 870, 'width' => 140, 'height' => 140),
					//'title'    => array('text' => $title, 'left' => 230, 'top' => 890, 'size' => 24, 'width' => 360, 'height' => 50, 'color' => '#FF0000'),
//					'price'    => array('text' => $price, 'left' => 270, 'top' => 930, 'size' => 30, 'color' => '#f20'),
					//'desc'     => array('text' => '长按二维码扫码购买', 'left' => 210, 'top' => 980, 'size' => 18, 'color' => '#666')
					//);
            }
			else 
	...
	public function get_comments() 
	{
		...
		$isagents = m("member")->getMember($_W['openid']);
		//$isagents['isagent'] = 0;
		show_json(1, array( "count" => $count, "percent" => $percent, "list" => $list,'isagent'=>$isagents['isagent'] ));
	}
